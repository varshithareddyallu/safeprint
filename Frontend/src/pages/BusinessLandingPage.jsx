...
const BusinessLandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 selection:bg-emerald-500/30 overflow-x-hidden">
      {/* Navbar */}
      <nav className="absolute top-0 w-full z-50 border-b border-slate-200 bg-slate-50/50 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate('/')}>
            <Store className="text-cyan-500 w-8 h-8" />
            <span className="font-extrabold text-slate-900 text-2xl tracking-tight">SafePrint<span className="text-cyan-500">Business</span></span>
          </div>
          <div className="flex items-center gap-6">
            <button onClick={() => navigate('/business/auth')} className="text-sm font-bold text-slate-700 hover:text-slate-900 transition">Sign In</button>
      ...
      <div className="relative pt-40 pb-20 lg:pt-56 lg:pb-32 px-6 max-w-7xl mx-auto flex flex-col items-center text-center">
        ...
        <p className="text-lg lg:text-xl text-slate-600 max-w-2xl mb-12 leading-relaxed">
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full justify-center max-w-md">
          ...
          <button onClick={() => navigate('/nearby')} className="flex-1 bg-slate-100 hover:bg-slate-700 text-slate-900 font-bold py-4 px-8 rounded-full border border-slate-300 transition flex items-center justify-center gap-2">
            View Live Map
          </button>
        </div>
      </div>
      ...
      <div className="py-24 bg-white border-y border-slate-200 relative z-10">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-600 max-w-xl mx-auto">We bring the power of machine learning and military-grade encryption directly to your local printing business.</p>
          </div>
      ...
      <div className="py-32 px-6 text-center max-w-4xl mx-auto">
        ...
        <p className="text-slate-600 mb-10 text-lg">Join the network of secure, ultra-efficient SafePrint partner shops today. Setup takes exactly 60 seconds.</p>
...
const FeatureCard = ({ icon, title, desc }) => (
  <div className="bg-slate-50 border border-slate-200 p-8 rounded-3xl hover:border-cyan-500/50 transition-colors group">
    <div className="w-16 h-16 bg-white border border-slate-200 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
      {icon}
    </div>
